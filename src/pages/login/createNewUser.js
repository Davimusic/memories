...
const SignUp = () => {
    ...
    try {
      ...
      try {
        ...
        if (createError.code === 'auth/email-already-in-use') {
          // Si el correo ya existe, intentar iniciar sesión
          userCredential = await signInWithEmailAndPassword(auth, email, password);
        } else {
          throw createError; // Lanzar otros errores
        }
      }
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      // Error ya manejado en handleGoogleLogin
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <BackgroundGeneric isLoading={isLoading}>
        <div className="login-container">
          <form onSubmit={handleSubmit} className="login-form">
            ...
            <button type="submit" className="submit-button" disabled={isLoading || !hasAgreedToTerms}>
              {isLoading ? <span className="loading-spinner">Validating...</span> : 'Sign Up'}
            </button>
            <button
              type="button"
