...
const MemoryCleaner = () => {
    ...
    const fetchMemoryData = async () => {
      try {
        ...
        const formattedData = {
          ...
          media: {
            photos: memory.media?.photos || [],
            videos: memory.media?.videos || [],
            audios: memory.media?.audios || [],
            documents: memory.media?.documents || [],
          },
        };
  ...
  useEffect(() => {
    if (permissionResult && permissionResult.media) {
      const media = permissionResult.media;
      setOriginalFiles({
