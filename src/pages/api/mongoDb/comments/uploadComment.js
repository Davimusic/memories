...
export default async function handler(req, res) {
  ...
  const { userId, uid, token, memoryId, uniqueMemoryId, text, parentId, root, fileId } = req.body;
...
export default async function handler(req, res) {
  ...
  const { userId, uid, token, memoryId, uniqueMemoryId, text, parentId, root } = req.body;
  ...
  // 2. Validate uniqueMemoryId for dynamicMemory
  if (root === 'dynamicMemory' && !uniqueMemoryId) {
    return res.status(400).json({
      success: false,
      message: 'uniqueMemoryId is required for dynamicMemory comments',
  ...
  try {
    ...
    // 7. Verify parent comment exists (if provided)
    if (parentId) {
      let commentsArray;
      if (root === 'dynamicMemory') {
        ...
        commentsArray = existingUser.files || [];
      }
      const parentComment = commentsArray.find((c) => c.id === parentId);
      if (!parentComment) {
        return res.status(404).json({
          success: false,
          message: 'Parent comment not found.',
    ...
    // 8. Initialize comments array if it doesn't exist
    const existingComments = root === 'dynamicMemory'
      ? existingUser[memoryKey]?.dynamicMemories?.[uniqueMemoryId]?.comments
      : root === 'generalMemory'
      ? existingUser[memoryKey].comments
      : existingUser.files;
    if (!existingComments) {
      await collection.updateOne(
        { _id: hashID },
