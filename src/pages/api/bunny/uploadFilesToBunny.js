...
// Función para crear estructura de directorios en BunnyCDN
async function createDirectoryHierarchy(BUNNY_ENDPOINT, BUNNY_ACCESS_KEY, folderPath) {
  const segments = folderPath.split('/').filter((s) => s); // eliminamos segmentos vacíos
  let currentPath = '';
  
  for (const segment of segments) {
    currentPath += `${segment}/`;
    try {
      // Codificamos cada segmento y reconstruimos el path, asegurándonos de omitir elementos vacíos
      const encodedPath =
        currentPath
          .split('/')
          .filter(Boolean)
      ...
      if (!response.ok) {
        ...
        // Si la respuesta es 400 y el mensaje indica que el directorio ya existe, se ignora el error
        if (response.status === 400 && errorText.toLowerCase().includes('already exists')) {
          console.log(`Directorio ${currentPath} ya existe, omitiendo creación.`);
        } else {
          console.error(
            `Error creando directorio ${currentPath}: ${response.status} - ${errorText}`
          );
          // Si prefieres detener el proceso en caso de otro error, podrías lanzar un Error aquí:
          // throw new Error(`Error creando directorio ${currentPath}: ${response.status} - ${errorText}`);
        }
      }
    } catch (error) {
...
// Función para extraer nombre y extensión del archivo
    ...
    for (const file of filesArray) {
      try {
        ...
        try {
          ...
          // Ignorar error al borrar el archivo temporal
        }
      }
    }
