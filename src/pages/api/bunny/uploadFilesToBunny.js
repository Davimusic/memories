...
// Función para crear jerarquía de directorios
async function createDirectoryHierarchy(BUNNY_ENDPOINT, BUNNY_ACCESS_KEY, folderPath) {
...
export default async function handler(req, res) {
  ...
  const BUNNY_ENDPOINT = `https://${BUNNY_REGION}.storage.bunnycdn.com/${STORAGE_ZONE}/`;
  ...
  try {
    ...
    // Obtener parámetros de la nueva estructura
    ...
    for (const file of filesArray) {
      try {
        const fileBuffer = readFileSync(file.filepath);
        const originalName = encodeURIComponent(file.originalFilename);
        ...
        // Generar URL pública
        //const publicUrl = `https://${BUNNY_REGION}.b-cdn.net/${STORAGE_ZONE}/${finalFileName}`;
        ...
        console.error(`Error subiendo archivo ${file.originalFilename}:`, error);
        uploadedFiles.push({
        ...
        try { unlinkSync(file.filepath); } catch (e) { console.error('Error eliminando temporal:', e); }
      }
    }
