...
const Menu = ({ isOpen, onClose, className = '', openUpdateBackgroundColor }) => {
  ...
  {['Memories', 'New memory', 'Favorites', 'Settings'].map((item, index) => (
    <li key={index} style={{ marginBottom: '15px' }}>
      <a
        href={item === 'Memories' ? '/memories' : '/#'}
        style={{ textDecoration: 'none' }}
        className="title-sm color2"
      >
        {item}
      </a>
    </li>
  ...
  <li style={{ marginBottom: '15px' }}>
    <p onClick={handleLogout} className="title-sm color2" style={{ cursor: 'pointer' }}>
      Log out
    </p>
  </li>
</ul>
