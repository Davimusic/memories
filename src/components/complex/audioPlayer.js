...
import CommentsIcon from './icons/commentsIcon'; // Asegúrate de que la ruta sea correcta
...
export default function AudioPlayer({
  ...
  const [commentsModalOpen, setCommentsModalOpen] = useState(false); // Estado para el modal de comentarios
  ...
  useEffect(() => {
    if (trackRefs.current[currentTrackIndex] && playlistContainerRef.current) {
      const activeElement = trackRefs.current[currentTrackIndex];
      const container = playlistContainerRef.current;
      const containerHeight  = container.clientHeight;
      const elementOffset = activeElement.offsetTop;
      const elementHeight = activeElement.clientHeight;
      const isVisible = (
        elementOffset >= container.scrollTop &&
        elementOffset + elementHeight <= container.scrollTop + containerHeight
      );
      if (!isVisible) {
        container.scrollTo({
  ...
  const handleCommentAdded = (newComment) => {
    onCommentAdded(currentTrackIndex, newComment);
  };
            ...
            <div className="playlist-container" ref={playlistContainerRef}>
              <ul className="playlist">
                {audioFiles.map((track, index) => (
                  <li
                    key={index}
                    ref={(el) => handleTrackRef(el, index)}
                    onClick={() => {
