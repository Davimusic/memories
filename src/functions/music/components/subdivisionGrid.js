...
import UploadAudiosFromDAW from '../../../components/complex/uploadFilesFromUsers';
...
const usePlayback = ({
  ...
  }, [stopPlayback, getCurrentPatternSteps, playSound, isPlayingRef, gridContainerRef, measureWidthRef, measuresRef, numeratorRef, subdivisionsPerPulseRef]);
...
const useSamples = ({ isPianoMode, PREDEFINED_PIANO_SAMPLES }) => {
  ...
  const loadSamples = useCallback(async () => {
    if (isPianoMode) {
      try {
        if (!PREDEFINED_PIANO_SAMPLES.length) {
          setSamplesLoaded(false);
          return;
        }

        const sample = PREDEFINED_PIANO_SAMPLES[0];
        if (!globalAudioContextRef.current || globalAudioContextRef.current.state === 'closed') {
          globalAudioContextRef.current = new (window.AudioContext || window.webkitAudioContext)();
        }
  ...
  }, [isPianoMode, PREDEFINED_PIANO_SAMPLES]);
...
const Grid = ({
  ...
  const totalHeight = rowPositions[rows - 1] + getEffectiveRowHeight(rows - 1);

  const is88KeyMode = (isPianoMode || mode === 'piano') && rows === 88;
    ...
    return Array.from({ length: rows }).map((_, rowIndex) =>
      hiddenRows.has(rowIndex) ? (
        ...
        >
          ...
          >
            {(isPianoMode || mode === 'piano') ? pianoNotes[rowIndex] : `Row ${rowIndex + 1}`}
          </div>
        </div>
      ) : (
        <div
          ...
          style={{
            ...
            fontSize: '10px',
            color: '#000',
            cursor: (mode === 'percussion' || !isPianoMode) ? 'pointer' : 'default',
            userSelect: 'none',
            boxSizing: 'border-box',
      ...
      if (hiddenRows.has(rowIndex)) {
        ...
        return Array.from({ length: totalSteps }).map((_, stepIndex) => {
          ...
          return (
            ...
            >
              {isActive && (isPianoMode || mode === 'piano') && (
                <div
                  style={{
                    position: 'absolute',
                    top: '50%',
...
function RowControlsModal({
  ...
  // Usar rowSamples[rowIndex] para obtener el sample actual
  const [selectedColor, setSelectedColor] = useState(
    customColors[currentSample] || DEFAULT_COLORS[currentSample]
  );

  // Actualizar cuando cambien los props
  useEffect(() => {
    setSelectedColor(customColors[currentSample] || DEFAULT_COLORS[currentSample]);
  }, [currentSample, customColors]);
  ...
  return (
    <div style={{ 
      ...
      <div
        style={{
          display: 'flex',
          width: `${measureWidth + (showLeftPanel ? 60 : 0)}px`,
          border: '1px solid #ddd',
          backgroundColor: '#f5f5f5',
          borderRadius: '4px',
          boxSizing: 'border-box'
      ...
      >
        ...
        >
          <div style={{ width: `${totalGridWidth*2}px`, minWidth: '100%', boxSizing: 'border-box' }}>
            <div
              style={{
                display: 'grid',
                gridTemplateColumns: `repeat(${totalSteps}, ${cellWidth}px)`,
